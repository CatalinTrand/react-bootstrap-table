...
export const getFilterFnForColumn = (column: ColumnConfig<any>, filterMode: FilterMode) => {
  ...
  switch (filter) {
    case 'as-boolean':
      ...
      break;
    case 'as-text':
      filterFn = filterTextFn(referenceValue, condition);
      break;
    case 'as-number':
...
export const filterTextFn = (ref: CellValueType, condition: 'contains' | 'le' | 'ge' | 'eq') => {
  let filterFn: (el: any) => boolean;
  //TODO
  return filterFn;
}

export const filterNumberFn = (ref: CellValueType, condition: 'contains' | 'le' | 'ge' | 'eq') => {
  let filterFn: (el: any) => boolean;
  let _referenceValue = parseFloat(String(ref));
  if(isNaN(_referenceValue)){
    filterFn = (el: any) => false;
  } else {
    switch (condition) {
      case 'eq':
        filterFn = (el: any) =>
...
export const filterDateFn = (ref: CellValueType, condition: 'contains' | 'le' | 'ge' | 'eq') => {
  let filterFn: (el: any) => boolean;
  //TODO
  return filterFn;
}
